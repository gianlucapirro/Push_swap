...
class PsGui:
    def __init__(self, master):
        ...
        self.i = 0
        self.speed = 0
        randomlist = [x for x in random.sample(range(-1999234, 231432141), 300)]
        dirname = os.path.dirname(os.path.abspath(__file__))
        PUSHS_PATH = os.path.join(dirname, RELATIVE_PATH)
        self.pile_a = randomlist
        self.first_pile = self.pile_a[:]
        self.pile_b = []
        self.cmds = subprocess.check_output([PUSHS_PATH] + list(map(str, randomlist)), stderr=subprocess.STDOUT,
